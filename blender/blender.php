...
bpy.data.textures['$textureName'].image = ".$textureName."TextureChange";
	...
	public function renderAvatar( array $colors = [], array $objNames = [], array $misc = [], array $clothing = [], array $face = []) {
		...
		if (isset($misc["filename"])) {
			...
			$hecc = rand(0,100);
			$fileName = "/var/www/html/assets/avatar/" . "image_" . $hecc;
		}
		$python = "import bpy";
		...
		if($clothing['pants'] == false){
			$this->changeTexture($python, "LeftLegTexture", "/var/www/html/blender/middle.png");
			$this->changeTexture($python, "RightLegTexture", "/var/www/html/blender/middle.png");
		} else {
			$this->changeTexture($python, "LeftLegTexture", "/var/www/html/assets/hats/storage/".$clothing["LeftLegTexture"].".png");
			$this->changeTexture($python, "RightLegTexture", "/var/www/html/assets/hats/storage/".$clothing["RightLegTexture"].".png");
		}
		if($clothing['shirts'] == false){
			...
			$this->changeTexture($python, "LeftArmTexture", "/var/www/html/assets/hats/storage/".$clothing["LeftArmTexture"].".png");
			$this->changeTexture($python, "RightArmTexture", "/var/www/html/assets/hats/storage/".$clothing["RightArmTexture"].".png");
			$this->changeTexture($python, "TorsoTexture", "/var/www/html/assets/hats/storage/".$clothing["TorsoTexture"].".png");
		}
		if($face['id'] == "0"){
			$this->changeTexture($python, "Face", "/var/www/html/assets/hats/storage/default_face.png");
		} else {
			$this->changeTexture($python, "Face", "/var/www/html/assets/hats/storage/".$face['id'].".png");
		}
		//Hats
		...
		if ($result) {
			if ($result->num_rows > 0) {
				...
				foreach ($hats as $hat) {
					if ($avatar[$hat] > 0) {
						$hatValue = $avatar[$hat];
						$hatArray[$hat] = ["/var/www/html/assets/hats/storage/" . $hatValue . ".obj" =>  "/var/www/html/assets/hats/storage/" . $hatValue . ".png"];
					}
				}
				if($avatar['pant'] != "0"){
					$check_pant = true;
				...
				$render = $this->renderAvatar(["Head" => $avatar["headColor"], "Torso" => $avatar["torsoColor"], "RightArm" => $avatar["rightArmColor"], "LeftArm" => $avatar["leftArmColor"], "LeftLeg" => $avatar["leftLegColor"], "RightLeg" => $avatar["rightLegColor"]],
										$hatArray,["filename" => "/var/www/html/assets/users/" . $id], ["LeftArmTexture" => $avatar['shirt'], "RightArmTexture" => $avatar['shirt'], "TorsoTexture" => $avatar['shirt'], "LeftLegTexture" => $avatar['pant'], "RightLegTexture" => $avatar['pant'], "pants" => $check_pant, "shirts" => $check_shirt], ["id" => $avatar['face']]);
...
";
		...
		$this->changeTexture($python, "Face", "/var/www/html/assets/coolface.png");
...
";
		...
		$this->changeTexture($python, "LeftArmTexture", "/var/www/html/assets/hats/storage/".$id.".png");
		$this->changeTexture($python, "RightArmTexture", "/var/www/html/assets/hats/storage/".$id.".png");
		$this->changeTexture($python, "TorsoTexture", "/var/www/html/assets/hats/storage/".$id.".png");
		$this->save($python, "/var/www/html/shop/images/".$id);
		$this->run($python, "/var/www/html/shop/images/" . $id . ".py");
	}
...
";
		...
		$this->changeTexture($python, "LeftLegTexture", "/var/www/html/assets/hats/storage/".$id.".png");
		$this->changeTexture($python, "RightLegTexture", "/var/www/html/assets/hats/storage/".$id.".png");
		$this->save($python, "/var/www/html/shop/images/".$id);
		$this->run($python, "/var/www/html/shop/images/" . $id . ".py");
	}
...
";
		...
		$this->changeTexture($python, "Face", "/var/www/html/assets/hats/storage/".$id.".png");
		$this->save($python, "/var/www/html/shop/images/".$id);
		$this->run($python, "/var/www/html/shop/images/" . $id . ".py");
	}
}